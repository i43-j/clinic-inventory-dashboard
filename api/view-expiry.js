const N8N_VIEW_EXPIRY_WEBHOOK = 'https://i43-j.app.n8n.cloud/webhook/view-expiry';
...
export default async function handler(req, res) {
  ...
  try {
    console.log('=== VIEW EXPIRY PROXY DEBUG ===');
    console.log('Request body:', req.body);
    console.log('Authorization header:', req.headers.authorization);
    console.log('N8N webhook URL:', N8N_VIEW_EXPIRY_WEBHOOK);

    // 4) Proxy the request to n8n
    const n8nRes = await fetch(N8N_VIEW_EXPIRY_WEBHOOK, {
      method: 'POST',
      headers: {
    ...
    console.error('[api/view-expiry] proxy error:', err);
