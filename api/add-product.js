const N8N_ADD_PRODUCT_WEBHOOK = 'https://i43-j.app.n8n.cloud/webhook/add-product';
...
export default async function handler(req, res) {
  ...
  try {
    console.log('=== ADD PRODUCT PROXY DEBUG ===');
    console.log('Request body:', req.body);
    console.log('Authorization header:', req.headers.authorization);
    console.log('N8N webhook URL:', N8N_ADD_PRODUCT_WEBHOOK);

    // 4) Proxy the request to n8n
    const n8nRes = await fetch(N8N_ADD_PRODUCT_WEBHOOK, {
      method: 'POST',
      headers: {
    ...
    console.error('[api/add-product] proxy error:', err);
